...
module.exports = class Creator {
  ...
  async downloadFromGitlab(repo, tag) {
    ...
    const repository = `${config['repository']}:${config['repositoryProtocol']}://${config['repositoryDomain']}:${templateRepo.path_with_namespace}${tag ? '#' + tag : ''}`
        ...
        metalsmith(__dirname)
          ...
          .use((files, metalsmith, done) => {
            ...
            Reflect.ownKeys(files).forEach(async file => {
              if (file.includes('.js') || file.includes('package.json') || file.includes('.env.')) {
